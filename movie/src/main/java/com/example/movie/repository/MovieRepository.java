...
public interface MovieRepository extends JpaRepository<Movie, Long> {
    ...
    @Query(value = "SELECT * FROM MOVIE m LEFT JOIN (SELECT r.MOVIE_MNO, COUNT(*),  AVG(r.GRADE)  FROM REVIEW r " +
            "GROUP BY r.MOVIE_MNO) r1 ON m.MNO = r1.movie_mno LEFT JOIN (SELECT mi2.* FROM MOVIE_IMAGE mi2 " +
            "WHERE mi2.inum IN (SELECT min(mi.inum) FROM MOVIE_IMAGE mi GROUP BY mi.movie_mno)) A ON " +
            "m.mno = A.movie_mno ORDER BY mno desc", nativeQuery = true)
